...
def main():
    ...
    if uploaded_file is not None:
        ...
        if analysis_type == 'Single Electrode':
            ...
            fig, ax = plt.subplots()
            ax.plot(time[:len(processed_data)+1], processed_data, label=f'{electrode} - 필터링된 신호')
            ...
            plt.figure(figsize=(1800, 100))
            fig, ax = plt.subplots()
            ax.plot(time[:len(processed_data1)+1], processed_data1, label=f'{electrode1} - 필터링된 신호')
            ax.plot(time[:len(processed_data2)+1], processed_data2, label=f'{electrode2} - 필터링된 신호')
            ax.set_xlabel('Time (s)')
            ax.set_ylabel('Amplitude')
            ax.legend()
            st.pyplot(fig)
            ...
            plt.figure(figsize=(1800, 100))
            fig, ax = plt.subplots()
            ax.plot(time[:len(difference)+1], difference, label='Difference')
            ax.set_xlabel('Time (s)')
